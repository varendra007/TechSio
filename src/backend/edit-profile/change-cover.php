<?php
    ...
    if(isset($_FILES['cover-image'])){
      ...
      // echo "37";
      $img_name = $_FILES['cover-image']['name']; // name of image file
      $temp_name = $_FILES['cover-image']['tmp_name']; // this tmeporary name is used to save/move file in our folder
      ...
      if(in_array($img_ext, $valid_extenstions) === true){
        ...
        if(move_uploaded_file($temp_name, "../images/cover/".$new_image_name)){
            
          $sql = "UPDATE user_info SET cover_image = '{$new_image_name}' WHERE user_id ='{$unique_id}'";
          if($conn->query($sql)){
            echo "success";
          }else{
            echo "Unable to change Cover image";
          }
